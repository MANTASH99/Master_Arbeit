...
linear_reg_model.fit(input_train, output_train)

# Make predictions on the test set
output_pred = linear_reg_model.predict(input_test)

# Calculate Mean Absolute Error (MAE) loss
mae_loss = mean_absolute_error(output_test, output_pred)
...
plt.scatter(output_test, output_pred, color='blue')
plt.plot([output_test.min(), output_test.max()], [output_test.min(), output_test.max()], 'r--', lw=2)
plt.xlabel('WRS65')
