...
decision_tree_model.fit(input_train, output_train)

# Make predictions on the test set
output_pred_dt = decision_tree_model.predict(input_test)

# Calculate Mean Absolute Error (MAE) loss
mae_loss_dt = mean_absolute_error(output_test, output_pred_dt)
print("Decision Tree Mean Absolute Error (MAE):", mae_loss_dt)

# Calculate R2 score
r2_dt = r2_score(output_test, output_pred_dt)
print("Decision Tree R2 Score:", r2_dt)
plt.figure(figsize=(10, 6))
plt.scatter(output_test, output_pred_dt, color='green', label='EV65 vs. Predicted')
#plt.plot([output_test.min(), output_test.max()], [output_test.min(), output_test.max()], 'k--', lw=2, color='red', label='Line of Best Fit')
plt.xlabel('EV65')
plt.ylabel('Predicted EV65')
